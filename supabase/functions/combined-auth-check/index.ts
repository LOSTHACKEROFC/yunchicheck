...
const notifyChargedCard = (
  ...
  }).catch((err) => console.error("[COMBINED-AUTH] notify-charged-card error:", err));
};
...
serve(async (req) => {
  ...
  try {
    ...
    console.log('[COMBINED-AUTH] Checking card for user:', user.id);
    ...
    if (finalResult.computedStatus === 'live') {
      notifyChargedCard(
        user.id,
        cc,
        'CHARGED',
