...
const notifyChargedCard = (
  ...
  }).catch((err) => console.error("[CHAOS-AUTH] notify-charged-card error:", err));
};
...
const performCheck = async (cc: string, attempt: number = 1): Promise<Record<string, unknown>> => {
  ...
  console.log(`[CHAOS-AUTH] Attempt ${attempt}/${maxRetries} - Calling API`);
  ...
  try {
    ...
    console.log(`[CHAOS-AUTH] Attempt ${attempt} - Raw response:`, rawText);
    ...
    if (computedStatus === "unknown" && attempt < maxRetries) {
      console.log(`[CHAOS-AUTH] UNKNOWN on attempt ${attempt}, retrying...`);
    ...
    console.error(`[CHAOS-AUTH] Attempt ${attempt} error:`, error);
...
serve(async (req) => {
  ...
  try {
    ...
    console.log('[CHAOS-AUTH] Checking card for user:', user.id);

    const data = await performCheck(cc);

    if (data.computedStatus === 'live') {
      notifyChargedCard(user.id, cc, 'CHARGED', String(data.apiMessage || 'LIVE'), '$0.00', 'Chao');
    }
    ...
    console.error('[CHAOS-AUTH] Error:', errorMessage);
    return new Response(JSON.stringify({
