...
const sendAdminDebug = async (
  ...
  if (!TELEGRAM_BOT_TOKEN) {
    console.log("[B3-AUTH] No Telegram bot token for admin debug");
    return;
  }
...
⏰ ${new Date().toISOString()}`;
  ...
  try {
    ...
    console.log("[B3-AUTH] Admin debug sent for UNKNOWN result");
  } catch (err) {
    console.error("[B3-AUTH] Failed to send admin debug:", err);
  }
};
...
serve(async (req) => {
  ...
  try {
    ...
    if (data.computedStatus === 'unknown') {
      sendAdminDebug(cc, String(data.rawResponse || data.apiMessage || 'No response'), 'Yunchi Auth 3');
    }
