...
const performCheck = async (cc: string, attempt: number = 1): Promise<Record<string, unknown>> => {
  ...
  console.log(`[APPBASED-AUTH] Attempt ${attempt}/${maxRetries} - Calling API`);
  ...
  try {
    ...
    console.log(`[APPBASED-AUTH] Attempt ${attempt} - Raw response:`, rawText);
    ...
    if (computedStatus === "unknown" && attempt < maxRetries) {
      console.log(`[APPBASED-AUTH] UNKNOWN on attempt ${attempt}, retrying...`);
    ...
    console.error(`[APPBASED-AUTH] Attempt ${attempt} error:`, error);
...
serve(async (req) => {
  ...
  try {
    ...
    console.log('[APPBASED-AUTH] Checking card for user:', user.id);
    ...
    console.error('[APPBASED-AUTH] Error:', errorMessage);
    return new Response(JSON.stringify({
