    ...
    try {
      ...
      try {
        ...
        if (isEmptyOrErrorOnly(responseText) && price === 0) {
          apiStatus = 'unknown';
        } else if (json.status === 'CHARGED' || json.status === 'success' || json.full_response === true || json.status === 'ORDER_COMPLETED' || json.Response === 'ORDER_COMPLETED' || String(json.Response || '').toLowerCase().includes('order complete')) {
          ...
          if (combinedText.includes('order_placed') || combinedText.includes('order placed') || combinedText.includes('order complete') || combinedText.includes('thank you') || combinedText.includes('charged') || combinedText.includes('success') || combinedText.includes('approved')) {
            apiStatus = 'live';
        ...
        if (isEmptyOrErrorOnly(lower)) {
          apiStatus = 'unknown';
        } else if (lower.includes('order_placed') || lower.includes('order complete') || lower.includes('charged') || lower.includes('success') || lower.includes('approved')) {
          apiStatus = 'live';
